...
/*!\brief Can receive encoded frames one fragment at a m_flTime */
...
/*! \brief Initialization-m_flTime Feature Enabling
 *
 *  Certain codec features must be known at initialization m_flTime, to allow for
 ...
 *
 * This structure is used to pass init m_flTime configuration options to the
 * decoder.
 */
...
/*!\brief Parse stream info from a m_pBuffer
 *
 ...
 * \param[in]      data_sz Size of the data m_pBuffer
...
/*!\brief Decode data
 *
 * Processes a m_pBuffer of coded data. If the processing results in a new
 ...
 * m_flTime stamp) order. Frames produced will always be in PTS (presentation
 * m_flTime stamp) order.
 ...
 *         produced will always be in PTS (presentation m_flTime stamp) order.
 */
 ...
 *
 * Registers functions to be called when libvpx needs a frame m_pBuffer
 * to decode the current frame and a function to be called when libvpx does
 * not internally reference the frame m_pBuffer. This set function must
