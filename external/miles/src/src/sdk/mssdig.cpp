...
S32 setup_directsound(HDIGDRIVER dig)
{
   ...
   if ((dig->dsHwnd==0) || (!IsWindow((HWND)dig->dsHwnd)))
      {
          HWND hwnd;
          AIL_platform_property(dig, WIN32_HWND, 0, 0, &hwnd);

          dig->dsHwnd=(UINTa)hwnd;
      }
...
// Update the HWND associated with the directsound buffer with the HWND in the driver.
//-----------------------------------------------------------------------------
...
{
   S32 rel;
   InMilesMutex();    // Lock to avoid DirectSound deadlocks with SS_serve
                               ...
                               UINT          wSize)
{
   ...
   lpCaps->szPname[0]=0;
   if ( dig->emulated_ds )
     AIL_strcpy( lpCaps->szPname, "Emulated DirectSound - " );

   if (dig->voice_filter != NULL)
      AIL_strcat(lpCaps->szPname, "DirectSound - Voice filter");
   else if (dig->matrix_filter != NULL)
      AIL_strcat(lpCaps->szPname, "DirectSound - Matrix filter");
   else
      AIL_strcat(lpCaps->szPname, "DirectSound - MSS software mixer");
                         ...
                         DWORD           fdwOpen)
{
   ...
   if ((!AIL_preference[DIG_DS_USE_PRIMARY]) || (dig->emulated_ds))
      ...
      S32 n = dig->wformat.wf.nBlockAlign * 256;  
...
static void win_fill(HDIGDRIVER dig, void FAR *lpData)
{
  SS_fill( dig, lpData );
                                ...
                                LPWAVEFORMAT     lpFormat)
{
   ...
   *drvr = dig;
   ...
   if (dig->physical_channels_per_sample == 0)
      ...
      nch = dig->logical_channels_per_sample;
...
outofmem:
         ...
         AIL_mem_free_lock((void FAR *) dig->reverb[0].reverb_build_buffer);
         AIL_mem_free_lock((void FAR *) dig->samples);
...
{
    HDIGDRIVER result = 0;
    AIL_set_error("Failed to open multichannel driver");
    ...
    if (channel_spec == MSS_MC_USE_SYSTEM_CONFIG)
    {
        ...
        {
            AIL_set_error("Failed to get speaker configuration");
            return 0;
        }
    }
...
tryagain:
   ...
   if (result != 0)
      ...
      if (!filter_OK)
         ...
         AIL_set_error("Failed to associate driver with output filter");
         result = 0;
         }
      }
#endif
