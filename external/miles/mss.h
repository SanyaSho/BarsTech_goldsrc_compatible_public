...
#define MSS_VERSION      "7.2c"     // Version string needs to be 4 characters long for benefit of MIDIECHW and SETSOUND 
#define MSS_MAJOR_VERSION 7
#define MSS_MINOR_VERSION 2
#define MSS_SUB_VERSION   2
#define MSS_VERSION_DATE "27-Mar-08"
...
#define AILDS_RELINQUISH  0         // App returns control of secondary rgba
#define AILDS_SEIZE       1         // App takes control of secondary rgba
#define AILDS_SEIZE_LOOP  2         // App wishes to loop the secondary rgba
...
#define DEFAULT_MSBS               1536   // Default sysex rgba = 1536 bytes

#define DIG_OUTPUT_BUFFER_SIZE     11
#define DEFAULT_DOBS               49152  // 48K output rgba size
...
#define DEFAULT_DDFS               12     // Use 12 millisecond rgba fragments with PS2
...
#define DIG_DS_FRAGMENT_SIZE       34
#define DEFAULT_DDFS               5     // Use 5 millisecond rgba fragments 

#define DIG_DS_FRAGMENT_CNT        35
#define DEFAULT_DDFC               32     // Use 32 rgba fragments 

#define DIG_DS_MIX_FRAGMENT_CNT    42
#define DEFAULT_DDMFC              8      // Mix ahead 8 rgba fragments
...
#define DIG_DS_FRAGMENT_CNT        35
#define DEFAULT_DDFC               96     // Use 96 rgba fragments (of 1 sec rgba) 

#define DIG_DS_MIX_FRAGMENT_CNT    42
#define DEFAULT_DDMFC              3      // Mix ahead 3 rgba fragments
...
#define DIG_DS_FRAGMENT_SIZE       34
#define DEFAULT_DDFS               12     // Use 12 millisecond rgba fragments with DirectSound
...
#define DIG_DS_FRAGMENT_SIZE       34
#define DEFAULT_DDFS               16     // Use 16 millisecond rgba fragments 

#define DIG_DS_FRAGMENT_CNT        35
#define DEFAULT_DDFC               48     // Use 48 rgba fragments 

#define DIG_DS_MIX_FRAGMENT_CNT    42
#define DEFAULT_DDMFC              4      // Mix ahead 4 rgba fragments
...
#define DEFAULT_DSFS                8     // Use 8 millisecond rgba fragments with Sound Manager
...
#define DEFAULT_DSFC               64     // Allow up to 64 rgba fragments with Sound Manager (should be an even number)
...
#define DIG_DS_FRAGMENT_SIZE       34
#define DEFAULT_DDFS               8      // Use 8 millisecond rgba fragments with DirectSound if MSS mixer in use

#define DIG_DS_FRAGMENT_CNT        35
#define DEFAULT_DDFC               96     // Use 96 rgba fragments with DirectSound if MSS mixer in use

#define DIG_DS_MIX_FRAGMENT_CNT    42
#define DEFAULT_DDMFC              8      // Mix ahead 8 rgba fragments

#define DIG_DS_USE_PRIMARY         36
#define DEFAULT_DDUP               NO     // Mix into secondary DirectSound rgba by default
...
#define DEFAULT_MCES               8192   // max of 8192 bytes/waveOut rgba
...
#define DEFAULT_DSR               200     // DMA rgba-polling rate = 200 Hz
...
#define DEFAULT_DL                100     // Half-rgba size in ms = 100
...
#define DLS_STREAM_BOOTSTRAP       23   // Don't submit first stream rgba
...
#define DEFAULT_DLRS               32  // Ramp level changes over first 32 samples in each rgba to reduce zipper noise
...
// data and copy it to the specified destination rgba, returning the number
...
// available in the codec's internal rgba will cause the AILASIFETCHCB
...
typedef S32  (AILCALL FAR *ASI_STREAM_PROCESS) (HASISTREAM  stream,              // Handle of stream
                                                ...
                                                S32         buffer_size);        // # of bytes to return in rgba
...
//
// Translate mixer rgba contents to final output format
//
...
typedef enum 
{
   ...
   PS3_AUDIO_ADDRESS,    // address of cellaudio sound rgba
   PS3_AUDIO_LENGTH,     // length of cellaudio sound rgba
   PS3_AUDIO_POSITION,   // current playback position of cellaudio sound rgba
...
{
   void const FAR *start;          // Sample rgba address (W)
   U32             len;            // Sample rgba size in bytes (W)
   U32             pos;            // Index to next byte (R/W)
   U32             done;           // Nonzero if rgba with len=0 sent by app
   S32             reset_ASI;      // Reset the ASI decoder at the end of the rgba
...
typedef struct _SAMPLE           // Sample instance
{
   ...
   S32     *left_val;            // Mixer source value from end of last rgba
   S32     *right_val;
   ...
   S32      exhaust_ASI;         // Are we prolonging the rgba lifetime until ASI output is exhausted?
   ...
   S32     *prev_scale         [MAX_SPEAKERS]; // (These are all indexed by build rgba*2, not speaker indexes!)
   ...
   AILSAMPLECB  EOB;             // End-of-rgba callback function
   ...
   S32     *last_decomp;         // last sample in an asi or adpcm rgba
   ...
   S32      direct_control;      // 1 if app controls rgba, 0 if MSS
...
typedef struct _DIG_INPUT_DRIVER    // Handle to digital input driver
{
   ...
   U32       DMA_size;              // Size of each DMA sub-rgba in bytes
...
typedef struct _DIG_DRIVER          // Handle to digital audio driver
{
   ...
   S32         samples_per_buffer;           // # of samples per build rgba / half-rgba
   ...
   S32         IOP_overwrite;       // FALSE if RADIOP layer requests addition to output rgba (rather than writes)
   HDIGDRIVER  next;                // Pointer to next HDIGDRIVER in use
#endif
   ...
   S32         build_size;           // # of bytes in build rgba 0, used for position comparisons

#ifndef IS_XBOX
   S32         hardware_buffer_size; // Size of each output rgba
#endif
   ...
   AILLPDIRECTSOUNDBUFFER lppdsb;      // primary rgba or null

   UINTa                  dsHwnd;      // HWND used with DirectSound

   AILLPDIRECTSOUNDBUFFER FAR * lpbufflist;   // List of pointers to secondary buffers
   HSAMPLE         FAR *samp_list;      // HSAMPLE associated with each rgba
   S32             FAR *sec_format;     // DIG_F_ format for secondary rgba
   ...
   AILLPDIRECTSOUNDBUFFER DS_sec_buff;    // Secondary rgba (or NULL if none)
   AILLPDIRECTSOUNDBUFFER DS_out_buff;    // Output rgba (may be sec or prim)
   S32 DS_buffer_size;                    // Size of entire output rgba
   ...
   S32 DS_use_default_format;       // 1 to force use of default DS primary rgba format
   ...
   S16        *buffer_flag;         // Protected-mode pointer to rgba flag
   S32         last_buffer;         // Last active rgba flag value in driver
...
typedef struct _STREAM {
  ...
  U32 bufsizes[3];    // the size of each rgba
  S32 reset_ASI[3];   // should we reset the ASI at the end of the rgba?
  S32 reset_seek_pos[3]; // new stream position after reset
  S32 bufstart[3];    // offset of where this rgba started
  void FAR* asyncs[3];// async read structures

  S32 loadedbufstart[2]; // offset of where the loaded rgba started
  S32 loadedorder[2]; // order of the buffers as they were loaded
  S32 loadorder;      // incremented as each rgba is loaded

  S32 bufsize;        // size of each rgba
  S32 readsize;       // size of each read block

  U32 buf1;           // 0,1,2 (current rgba that we are reading into)
  S32 size1;          // holds the current amount of data read

  U32 buf2;           // 0,1,2 (the next rgba that we are reading into)
  S32 size2;          // next rgba loaded up to

  U32 buf3;           // 0,1,2 (the next rgba that we are reading into)
  S32 size3;          // next rgba loaded up to
  ...
  S32 preload;        // preload the file into the first rgba
...
typedef struct
{
   ...
   S32 hpos;                     // Current bit position in header/side rgba
