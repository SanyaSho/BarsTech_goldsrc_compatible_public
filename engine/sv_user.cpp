...
void SV_ParseMove(client_t *pSenderClient)
{
	...
	DbgPrint(m_fMessages, "full move packet is:\r\n");
	for (int i = 0; i < net_message.cursize; i++)
		DbgPrint(m_fMessages, "%02X ", net_message.data[i]);
	...
	DbgPrint(m_fMessages, "packetLossByte %d numbackup %d numcmds %d\r\n", packetLossByte, numbackup, numcmds);
...
void SV_ExecuteClientMessage(client_t* cl)
{
	...
	while (1)
	{
		...
		DbgPrint(m_fMessages, "received %d message from client\r\n", c);
