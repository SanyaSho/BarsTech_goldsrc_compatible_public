...
int W_LoadWadFile( const char* filename )
{
	...
	{
		Con_Printf( const_cast<char*>("No room for wad %s\n"), filename );
		return -1;
	}

	wadlist_t* pList = &wads[ iWadIndex ];

	pList->wad_base = COM_LoadHunkFile( (char*)filename );
	if( !pList->wad_base )
	{
		if( iWadIndex == 0 )
			Sys_Error( "W_LoadWadFile: couldn't load %s", filename );

		Con_Printf( const_cast<char*>("WARNING:  W_LoadWadFile, couldn't load %s\n"), filename );
		return -1;
	}
