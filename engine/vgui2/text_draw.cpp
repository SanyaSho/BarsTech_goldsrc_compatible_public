...
vgui2::HFont VGUI2_GetConsoleFont()
{
	return _consoleFont;
}
...
const wchar_t* VGUI2_Find_String( const char* str )
{
	...
	if( str && *str == '#' )
	{
		wchar_t* pszLocalized = vgui2::localize()->Find( str );

		if (pszLocalized != NULL)
			return pszLocalized;
	}
...
int VGUI2_DrawStringReverseClient(int x, int y, const char* str, int r, int g, int b)
{
	...
	const wchar_t* pszString = VGUI2_Find_String(str);
	int length = wcslen(pszString);
	const wchar_t* pszEnd = &pszString[length - 2];

	//Use the width of the last character for metrics
	int a, z, c;
	vgui2::surface()->GetCharABCwide(VGUI2_GetCreditsFont(), pszString[length - 1]/**pszEnd*/, a, z, c);

	const auto charWidth = a + z + c;

	int endX = x - charWidth;

	//for (; pszEnd != pszString; --pszEnd)
	for (int i = 0; i < length; i++)
	{
		endX -= VGUI2_Draw_CharacterAdd(endX, y, pszString[length - i - 1]/**pszEnd*/, r, g, b, VGUI2_GetCreditsFont());
	}
