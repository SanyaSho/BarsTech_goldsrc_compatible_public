...
bool CFontTextureCache::AllocatePageForChar( int charWide, int charTall, int& pageIndex, int& drawX, int& drawY, int& twide, int& ttall )
{
	...
	if( pageIndex < 0 ||
		...
		(
			( pPage->nextY + iHeight ) >= pPage->tall && 
...
bool CFontTextureCache::GetTextureForChar( vgui2::HFont font, wchar_t wch, int* textureID, float* texCoords )
{
	...
	if( index == m_CharCache.InvalidIndex() )
	{
...
#else
		int size = fontWide * fontTall * 4;
		unsigned char* pDest = (unsigned char*)_alloca(size); // (size + 3) & ~3
#endif
		...
		cacheitem.texCoords[ 0 ] = static_cast<double>( drawX ) / (float)twide;
		cacheitem.texCoords[ 1 ] = static_cast<double>( drawY ) / (float)ttall;
		cacheitem.texCoords[ 2 ] = static_cast<double>( drawX + fontWide ) / (float)twide;
		cacheitem.texCoords[ 3 ] = static_cast<double>( drawY + fontTall ) / (float)ttall;
