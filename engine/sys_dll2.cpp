	...
	if( FileSystem_Init( basedir, filesystemFactory ) )
	{
		...
		if( videomode->Init( instance ) )
		{
			...
			if( game->Init(	instance ) )
			{
				...
				{
					OutputDebugStringA("playing 1\n");
					videomode->PlayStartupSequence();
					OutputDebugStringA("playing ended\n");
					while( true )
					{
...
int Sys_GetSurfaceCacheSize( int width, int height )
{
	...
	const int iCacheSize = COM_CheckParm(const_cast<char*>("-surfcachesize"));
...
#if !defined(GLQUAKE)
SurfaceCacheForResFn D_SurfaceCacheForRes = nullptr;
#endif
...
void Sys_InitMemory()
{
	...
	// request otherwise
	const int heapsizeIndex = COM_CheckParm( const_cast<char*>("-heapsize") );
