...
CRC32_t CRC32_Final( CRC32_t ulCRC )
{
...
int CRC_MapFile( CRC32_t* crcvalue, char* pszFileName )
{
	...
	{
		Con_Printf(const_cast<char*>("Could not read BSP header for map [%s].\n"), pszFileName);
		FS_Close(fp);
		return 0;
	}
...
//-----------------------------------------------------------------------------
// Purpose: Start MD5 accumulation.  Set bit count to 0 and rgba to mysterious initialization constants.
...
//-----------------------------------------------------------------------------
// Purpose: Update context to reflect the concatenation of another rgba full of bytes.
// Input  : *ctx - 
//			*buf - 
//			len - 
//-----------------------------------------------------------------------------
...
BOOL MD5_Hash_File( unsigned char digest[ 16 ], char *pszFileName, int bUsefopen, int bSeed, unsigned int seed[ 4 ] )
{
	auto hFile = FS_Open( pszFileName, "rb" );
	...
	if( uiSize > 0 )
	{
		...
		if( bSeed )
			MD5Update( &ctx, reinterpret_cast<const unsigned char*>( seed ), ARRAYSIZE( seed ) * sizeof( decltype( seed ) ) );

		byte chunk[ 1024 ];

		while( 1 )
		{
			const auto iBytesRead = FS_Read( chunk, min( ARRAYSIZE( chunk ), uiSize ), 1, hFile );

			if( iBytesRead > 0 )
			{
				uiSize -= iBytesRead;
				MD5Update( &ctx, chunk, iBytesRead );
			}

			if( FS_EndOfFile( hFile ) || uiSize <= 0 )
			{
				FS_Close( hFile );
				MD5Final( digest, &ctx );
