...
bool BUsesSDLInput()
{
	...
	if( BIsValveGame() ||
		strstr( g_szfullClientName, "valve/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "cstrike/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "tfc/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "dod/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "gearbox/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "bshift/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "czeror/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "czero/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "opfor/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "dmc/cl_dlls/client" ) ||
		strstr( g_szfullClientName, "richchet/cl_dlls/client" ) )
	{
		s_bUseRawInput = true;
	}
...
public:
	...
	bool OnlyLegacyRender(void) override;
...
private:
	...
	SDL_Renderer* m_hSDLRenderer = nullptr;
	bool m_bCreatedRenderer = false;
...
bool CGame::Shutdown()
{
	if (m_hSDLRenderer && m_bCreatedRenderer)
		SDL_DestroyRenderer(this->m_hSDLRenderer);
...
void SDL_SetAttributes(bool tryNewCtx)
{
	...
	if (tryNewCtx)
	{
		...
		SDL_GL_SetAttribute(SDL_GL_CONTEXT_PROFILE_MASK, 2);
	}
}

bool CGame::CreateGameWindow()
{
	if( COM_CheckParm( const_cast<char*>("-noontop") ) )
...
//	SDL_SetHint( "SDL_VIDEO_X11_XVIDMODE", "1" );
	...
	// no SDL_WINDOW_RESIZABLE since HL25
	Uint32 uiFlags = SDL_WINDOW_HIDDEN
#if defined(GLQUAKE)
		| SDL_WINDOW_OPENGL
#endif
		;

	if( COM_CheckParm( const_cast<char*>("-noborder") ) )
	...
	int width, height;
	if( LoadTGA( const_cast<char*>("game.tga"), buffer, 512 * 512, &width, &height ) )
	{
	...
	m_bCreatedRenderer = false;

#if !defined(GLQUAKE)
	m_hSDLRenderer = SDL_GetRenderer(m_hSDLWindow);
	if (m_hSDLRenderer == NULL)
		m_hSDLRenderer = SDL_CreateRenderer(m_hSDLWindow, -1, 0);

	if (m_hSDLRenderer != NULL)
	{
		m_bCreatedRenderer = true;
		ClearScreen();
	}
#endif
	...
	{
		SDL_SetAttributes(false);
		m_bLegacyRender = true;
		m_hSDLGLContext = SDL_GL_CreateContext( m_hSDLWindow );
	}
	...
	{
		r = 0;
		Con_Printf(const_cast<char*>("Failed to get GL RED size (%s)\n"), SDL_GetError());
	}
	if( SDL_GL_GetAttribute( SDL_GL_GREEN_SIZE, &g ) )
	{
		g = 0;
		Con_Printf(const_cast<char*>("Failed to get GL GREEN size (%s)\n"), SDL_GetError());
	}
	if( SDL_GL_GetAttribute( SDL_GL_BLUE_SIZE, &b ) )
	{
		b = 0;
		Con_Printf(const_cast<char*>("Failed to get GL BLUE size (%s)\n"), SDL_GetError());
	}
	if( SDL_GL_GetAttribute( SDL_GL_ALPHA_SIZE, &a ) )
	{
		a = 0;
		Con_Printf(const_cast<char*>("Failed to get GL ALPHA size (%s)\n"), SDL_GetError());
	}
	if( SDL_GL_GetAttribute( SDL_GL_DEPTH_SIZE, &depth ) )
	{
		depth = 0;
		Con_Printf(const_cast<char*>("Failed to get GL DEPTH size (%s)\n"), SDL_GetError());
	}
	Con_Printf(const_cast<char*>("GL_SIZES:  r:%d g:%d b:%d a:%d depth:%d\n"), r, g, b, a, depth);
	if( r <= 4 || g <= 4 || b <= 4 || depth <= 15 || gl_renderer && strstr( gl_renderer, "GDI Generic" ) )
			...
			case SDL_TEXTINPUT:
				{
					if( key_dest == key_message )
					{
						...
						Q_strncpy( &chat_buffer[chat_bufferlen], ev.edit.text, uiSpaceLeft);
			...
			case SDL_WINDOWEVENT:
				{
					...
					case SDL_WINDOWEVENT_CLOSE:
						...
						if (m_hSDLRenderer != NULL && m_bCreatedRenderer )
							SDL_DestroyRenderer(m_hSDLRenderer);
						m_hSDLRenderer = NULL;
						SDL_DestroyWindow( m_hSDLWindow );
						m_hSDLWindow = nullptr;
						return;
...
{
#ifndef DEDICATED
	if (COM_CheckParm(const_cast<char*>("-novid")) != 0 )//|| COM_CheckParm("-dev") != 0)
		return;

	Con_Printf(const_cast<char*>("Playing Startup Videos...\n"));

	int length = 0;

	char *pbuffer = (char *)COM_LoadFile(const_cast<char*>("media/StartupVids.txt"), 5, &length);
	...
	while (true)
	{
		ptoken = COM_Parse(ptoken);

		if (Q_strlen(com_token) <= 0)
			break;

		FS_GetLocalPath(com_token, szFileName, 4096);

		PlayAVIAndWait(szFileName);
	}
...
bool CGame::OnlyLegacyRender(void)
{
	return m_bLegacyRender;
}
...
void CGame::AppActivate( bool fActive )
{
	if( fActive )
	{
		...
		if( host_initialized )
		{
			...
			{
				Cmd_ExecuteString( const_cast<char*>("unpause"), src_command );
	...
	else
	{
		...
		if( host_initialized )
		{
			...
			if( VGuiWrap2_IsInCareerMatch() && cl.paused == false )
			{
				Cmd_ExecuteString( const_cast<char*>("setpause"), src_command );
...
void CGame::ClearScreen()
{
	...
	SDL_SetRenderDrawColor(m_hSDLRenderer, 0, 0, 0, 0);
	SDL_RenderClear(m_hSDLRenderer);
	SDL_RenderPresent(m_hSDLRenderer);
}
