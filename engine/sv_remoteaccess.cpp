...
const char* CServerRemoteAccess::LookupStringValue(const char *variable)
{
	...
	if (!Q_strcasecmp(variable, "map"))
		return sv.name;

	if (!Q_strcasecmp(variable, "playercount"))
	{
		int count = 0;
	...
	if (!Q_strcasecmp(variable, "maxplayers"))
	{
		_snprintf(s_ReturnBuf, sizeof(s_ReturnBuf)-1, "%d", svs.maxclients);
		return s_ReturnBuf;
	}

	if (!Q_strcasecmp(variable, "gamedescription"))
...
bool CServerRemoteAccess::LookupValue(const char *variable, CUtlBuffer &value)
{
	...
	if (!Q_strcasecmp(variable, "stats"))
	{
		char stats[512];
	...
	if (!Q_strcasecmp(variable, "banlist"))
	{
		GetUserBanList(value);
		return true;
	}

	if (!Q_strcasecmp(variable, "playerlist"))
	{
		GetPlayerList(value);
		return true;
	}

	if (!Q_strcasecmp(variable, "maplist"))
	{
		GetMapList(value);
		return true;
	}

	if (!Q_strcasecmp(variable, "uptime"))
	{
	...
	if (!Q_strcasecmp(variable, "ipaddress"))
	{
	...
	if (!Q_strcasecmp(variable, "mapcycle"))
	{
		int len;
...
void CServerRemoteAccess::SetValue(const char *variable, const char *value)
{
	...
	if (!Q_strcasecmp(variable, "map"))
	{
		Cbuf_AddText(const_cast<char*>("changelevel "));
		Cbuf_AddText((char*)value);
		Cbuf_AddText(const_cast<char*>("\n"));
		Cbuf_Execute();
	}
	else if (!Q_strcasecmp(variable, "mapcycle"))
	{
		f = FS_Open(mapcyclefile.string, "wt");
		if (!f)
		{
			Con_Printf(const_cast<char*>("Couldn't write to read-only file %s, using file _dev_mapcycle.txt instead.\n"), mapcyclefile.string);
			Cvar_DirectSet(&mapcyclefile, const_cast<char*>("_temp_mapcycle.txt"));
			f = FS_Open(mapcyclefile.string, "wt");
		}
	...
	else
	{
		...
		if (var)
			Cvar_DirectSet(var, const_cast<char*>(value));
	}
}
