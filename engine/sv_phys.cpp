...
qboolean SV_PushRotate(edict_t* pusher, float movetime)
{
	...
	if (pusher->v.avelocity[0] == 0.0f && pusher->v.avelocity[1] == 0.0f && pusher->v.avelocity[2] == 0.0f)
	{
		pusher->v.ltime += movetime;
		return TRUE;
	}
	...
	for (int i = 1; i < sv.num_edicts; i++)
	{
		...
		// try to rotate the contacted entity
		pusher->v.solid = SOLID_NOT;
		...
		if (SV_TestEntityPosition(check))
		{
			...
			// rotate back any entities we already moved
			for (int e = 0; e < num_moved; e++)
			{
...
void SV_Physics_Pusher(edict_t* ent)
{
	...
	if (movetime)
	{
		if (ent->v.avelocity[0] != 0.0f || ent->v.avelocity[1] != 0.0f || ent->v.avelocity[2] != 0.0f)
		{
			if (ent->v.velocity[0] != 0.0f || ent->v.velocity[1] != 0.0f || ent->v.velocity[2] != 0.0f)
			{
...
qboolean SV_CheckWater(edict_t* ent)
{
	...
	if (cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT)
	{
		...
		if (cont <= CONTENTS_CURRENT_0)
		{
			...
			VectorMA(ent->v.basevelocity, 150.0f * ent->v.waterlevel / 3.0f, current_table[CONTENTS_CURRENT_0 - cont], ent->v.basevelocity);
		}
	}
...
float SV_RecursiveWaterLevel(const vec_t* center, float mins, float maxs, int count)
{
	...
	if (SV_PointContents(test) == CONTENTS_WATER)
		return SV_RecursiveWaterLevel(center, mins, waterlevel, count);

	return SV_RecursiveWaterLevel(center, waterlevel, maxs, count);
}
...
void PF_WaterMove(edict_t *pSelf)
{
	...
	{
		VectorMA(pSelf->v.velocity, (-0.8f * (float)pSelf->v.waterlevel * (float)host_frametime), pSelf->v.velocity, pSelf->v.velocity);
	}
}
...
void SV_Physics_Step(edict_t *ent)
{
	...
	else
	{
		if (gGlobalVariables.force_retouch != 0.0f)
		{
			...
			// @SH_CODE: yes, it's hentacle in blast pit
