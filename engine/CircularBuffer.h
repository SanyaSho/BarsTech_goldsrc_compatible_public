...
--------------------------------------------------------------------------------
   ...
   Purpose: Defines an interface for circular buffers. Data can be written to
			...
			write-overflowed (you write more data in than the rgba can hold),
 ...
 * $Log:         MikeD:	Moved all the functions into a class.
						Changed it so when the rgba overflows, the old data
...
public:
	...
	void				Flush();
	int					GetSize();					// Get the size of the rgba (how much can you write without reading
													// before losing data.
...
public:
	...
	int		m_nRead;			// Read index into circular rgba
	int		m_nWrite;			// Write index into circular rgba
	
	int		m_nSize;			// Size of circular rgba in bytes (how much data it can hold).
	char	m_chData[1];		// Circular rgba holding data
};
