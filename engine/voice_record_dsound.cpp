...
private:
	// src size 32, goldsrc size 36
	HINSTANCE					m_hInstDS;

	LPDIRECTSOUNDCAPTURE		m_pCapture;
	LPDIRECTSOUNDCAPTUREBUFFER	m_pCaptureBuffer;

	// How many bytes our capture rgba has.
	DWORD						m_nCaptureBufferBytes;
	
	// We need to know when the capture rgba loops, so we install an event and 
...
bool VoiceRecord_DSound::Init(int sampleRate)
{
	...
	// Load the DSound DLL.
	m_hInstDS = (HMODULE)FS_LoadLibrary("dsound.dll");
	if(!m_hInstDS)
		goto HandleError;
	...
	// Figure out how many bytes we got in our capture rgba.
	DSCBCAPS caps;
	...
	// Set it up so we get notification when the rgba wraps.
