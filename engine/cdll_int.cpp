...
bool LoadSecureClient( const char* pszDllName )
{
	...
	CSysModule* ret = reinterpret_cast<CSysModule*>( NLoadBlobFile( pszDllName, &g_blobfootprintClient, &cl_funcs, true ) );
...
void ClientDLL_Init()
{
	...
	bool bSecureLoaded = LoadSecureClient( g_szfullClientName );

	if( !bSecureLoaded && !fClientLoaded )
	{
...
void ClientDLL_CheckStudioInterface( CSysModule* hClientDLL )
{
	...
	if( fClientLoaded )
	{
		...
		if( cl_funcs.pStudioInterface )
		{
			...
			{
				Con_DPrintf( const_cast<char*>("Couldn't get client .dll studio model rendering interface.  Version mismatch?\n") );
				R_ResetStudio();
			}
		}
	}
...
void ClientDLL_UpdateClientData()
{
	if( cls.state == ca_active && !cls.demoplayback )
	{
		...
		VectorCopy(pViewEnt->origin, cdat.origin);
...
void ClientDLL_PostRunCmd( local_state_t* from, local_state_t* to, usercmd_t* cmd, int runfuncs, double time, unsigned int random_seed )
{
	...
	{
		Cvar_SetValue( const_cast<char*>("cl_lw"), 0.0 );
	}
}
...
{
	RecEnghudPhysInfo_ValueForKey( key );
...
{
	RecEnghudGetMousePosition( mx, my );
	...
	if( !VideoMode_IsWindowed() )
	{
		...
		*mx = int((float)*mx * ((float)vW / (float)wW));
		*my = int((float)*my * ((float)vT / (float)wT));

		*mx = int((float)*mx + ((float)*mx - (float)vW / 2) * (GetXMouseAspectRatioAdjustment() - 1.0));
		*my = int((float)*my + ((float)*my - (float)vT / 2) * (GetYMouseAspectRatioAdjustment() - 1.0));
	}
}

int hudIsNoClipping()
{
	RecEnghudIsNoClipping();
...
{
	RecEngCL_LoadModel(modelname, index);

	for (int i = 1; cl.model_precache[i] != NULL; i++)
	{
		if (!Q_strcasecmp(modelname, cl.model_precache[i]->name))
		{
			if (index)
				*index = i;

			return CL_GetModelByIndex(i);
		}
	}
...
{
	RecEnghudGetSpritePointer( hSprite );
...
{
	RecEnghudPrecacheEvent(type, psz);
					   ...
					   int bparam1, int bparam2 )
{
	...
	if( !( flags & FEV_SERVER ) )
	{
		...
		VectorCopy(cl.simvel, eargs.velocity);
...
{
	RecEnghudWeaponAnim( iAnim, body );
...
{
	RecEngCOM_ParseFile( data, token );

	char* return_data = COM_Parse( data );
	Q_strncpy( token, com_token, 1024 );
...
{
	RecEngCOM_AddAppDirectoryToSearchPath( pszBaseDir, appName );

	COM_AddAppDirectory( const_cast<char*>(pszBaseDir), appName );
}

int ClientDLL_ExpandFileName( const char* fileName, char* nameOutBuffer, int nameOutBufferSize )
{
	RecEngClientDLL_ExpandFileName( fileName, nameOutBuffer, nameOutBufferSize );

	char buf[ 512 ];
	Q_strncpy( buf, fileName, ARRAYSIZE( buf ) - 1 );
...
{
	RecEngPlayerInfo_ValueForKey( playerNum, key );
...
void PlayerInfo_SetValueForKey( const char* key, const char* value )
{
	...
	{
		RecEngPlayerInfo_SetValueForKey( key, value );
...
{
	RecEngGetPlayerUniqueID( iPlayer, playerID );
...
{
	RecEngGetTrackerIDForPlayer( playerSlot );
...
{
	RecEngGetPlayerForTrackerID( trackerID );
...
void SDL_GetMousePos( POINT* ppt )
{
	...
	if( !VideoMode_IsWindowed() )
	{
		...
		x = int((float)x * ((float)vW / (float)wW));
		y = int((float)y * ((float)vT / (float)wT));

		x = int((float)x + float((float)x - (float)vW / 2) * (GetXMouseAspectRatioAdjustment() - 1.0));
		y = int((float)y + float((float)y - (float)vT / 2) * (GetYMouseAspectRatioAdjustment() - 1.0));
	}
...
cmd_function_t* GetFirstCmdFunctionHandle()
{
	return Cmd_GetFirstCmd();
}

uintptr_t GetNextCmdFunctionHandle( uintptr_t cmdhandle )
{
	return uintptr_t( reinterpret_cast<cmd_function_t*>( cmdhandle )->next );
}

const char* GetCmdFunctionName(uintptr_t cmdhandle )
{
...
{
	RecEngLocalPlayerInfo_ValueForKey( key );
...
{
	RecEngCL_CreateVisibleEntity(type, ent);

	if (type == ET_BEAM)
	{
		if (cl_numbeamentities < MAX_MODEL_BEAMS)
...
int ClientDLL_AddEntity(int type, cl_entity_s *ent)
{
	...
	if (!cl_funcs.pAddEntity)
		return 0;

	if (!cl_funcs.pAddEntity(type, ent, ent->model != NULL ? ent->model->name : ""))
		return 0;

	if (!CL_IsSpectateOnly() || CL_IsDevOverviewMode() || type == ET_BEAM || type == ET_TEMPENTITY || ent->curstate.movetype == MOVETYPE_FOLLOW)
		return CL_CreateVisibleEntity(type, ent);
