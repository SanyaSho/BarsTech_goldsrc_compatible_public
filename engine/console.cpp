...
cvar_t con_fastmode = { const_cast<char*>("con_fastmode"), const_cast<char*>("1") };
cvar_t con_notifytime = { const_cast<char*>("con_notifytime"), const_cast<char*>("1") };
cvar_t con_color = { const_cast<char*>("con_color"), const_cast<char*>("255 180 30"), FCVAR_ARCHIVE };
cvar_t con_shifttoggleconsole = { const_cast<char*>("con_shifttoggleconsole"), const_cast<char*>("0") };
cvar_t con_mono = { const_cast<char*>("con_mono"), const_cast<char*>("0"), FCVAR_ARCHIVE };
...
void Con_DPrintf( char* fmt, ... )
{
	...
	if( developer.value != 0.0 && ( scr_con_current == 0.0 || cls.state != 5 ) )
	{
		...
		// print debug messages to IDE --barspinOff
		OutputDebugStringA(msg);
	}
...
void Con_Debug_f()
{
	if( con_debuglog )
	{
		Con_Printf( const_cast<char*>("condebug disabled\n") );
		con_debuglog = false;
	}
	else
	{
		con_debuglog = true;
		Con_Printf( const_cast<char*>("condebug enabled\n") );
	}
}
...
void Con_SetTimes_f()
{
	if( Cmd_Argc() == 2 )
	{
		con_num_times = clamp( Q_atoi( (char*)Cmd_Argv( 1 ) ), CON_TIMES_MIN, CON_TIMES_MAX );
		...
		Con_Printf( const_cast<char*>("%i lines will overlay.\n"), con_num_times );
	}
	else
	{
		Con_Printf( const_cast<char*>("contimes <n>\nShow <n> overlay lines [4-64].\n%i current overlay lines.\n"), con_num_times );
	}
}
...
{
	Con_SafePrintf("trying to toggleconsole...\r\n");
...
void Con_MessageMode_f()
{
	if( VGuiWrap2_IsInCareerMatch() == CAREER_NONE )
	{
		...
		if( Cmd_Argc() == 2 )
		{
			Q_strncpy( message_type, (char*)Cmd_Argv( 1 ), ARRAYSIZE( message_type ) - 1 );
...
If the line width has changed, reformat the rgba.
================
*/
...
void Con_Init()
{
	con_debuglog = COM_CheckParm(const_cast<char*>("-condebug")) != 0;

	if( con_debuglog )
		FS_RemoveFile( "qconsole.log", nullptr );

	con_text = reinterpret_cast<char*>( Hunk_AllocName( CON_TEXTSIZE, const_cast<char*>("context") ) );
	...
	Con_DPrintf( const_cast<char*>("Console initialized.\n") );
	...
	Cmd_AddCommand( const_cast<char*>("contimes"), Con_SetTimes_f );
	Cmd_AddCommand( const_cast<char*>("toggleconsole"), Con_ToggleConsole_f );
	Cmd_AddCommand( const_cast<char*>("hideconsole"), Con_HideConsole_f );
	Cmd_AddCommand( const_cast<char*>("messagemode"), Con_MessageMode_f );
	Cmd_AddCommand( const_cast<char*>("messagemode2"), Con_MessageMode2_f );
	Cmd_AddCommand( const_cast<char*>("clear"), Con_Clear_f );
	Cmd_AddCommand( const_cast<char*>("condebug"), Con_Debug_f );
...
void Con_DrawNotify()
{
	...
	for( int i = con_current - con_num_times + 1; i <= con_current; ++i )
	{
		if( i >= 0 && con_times[ i % con_num_times ] )
		{
			if( ( realtime - con_times[ i % con_num_times ] ) <= con_notifytime.value )
			{
				...
				{
					x += Draw_Character( x, v, g_szNotifyAreaString[ j ], VGUI2_GetConsoleFont() );
				}
	...
	if( key_dest == key_message )
	{
		...
		xOffset = Draw_String( xOffset + 3, v, const_cast<char*>(":") );
...
void Con_SafePrintf( const char* fmt, ... )
{
	...
	Con_Printf( const_cast<char*>("%s"), msg );
...
void Con_NXPrintf( con_nprint_t* info, const char* fmt, ... )
{
	...
	if( info )
	{
		if (info->index >= 0 && info->index < CON_MAX_DEBUG_AREAS)
		{
