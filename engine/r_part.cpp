...
void R_EntityParticles(cl_entity_t* ent)
{
	...
	{
		sx = sin(cl.time * avelocities[i][0]);
		cx = cos(cl.time * avelocities[i][0]);
		sy = sin(cl.time * avelocities[i][1]);
		cy = cos(cl.time * avelocities[i][1]);
		sz = sin(cl.time * avelocities[i][2]);
		cz = cos(cl.time * avelocities[i][2]);
...
void R_FreeDeadParticles(particle_t** ppparticles)
{
	...
	for (kill = *ppparticles; kill != NULL; kill = kill->next)
	{
		...
		{
			if (queue == NULL || queue->die >= cl.time)
				break;
...
void R_ParticleExplosion2(vec_t* org, int colorStart, int colorLength)
{
	...
	for (int i = 0; i < 512; i++)
	{
		...
		p->die = cl.time + 0.3;
		p->color = colorMod % colorLength + colorStart;
#if defined( GLQUAKE )
		p->packedColor = 0;
#else
		p->packedColor = hlRGB(host_basepal, p->color);
...
void R_BlobExplosion(vec_t* org)
{
	...
	for (int i = 0; i < 1024; i++)
	{
		...
		ap = free_particles;
		free_particles = ap->next;
		ap->next = active_particles;
		active_particles = ap;

		ap->die = RandomFloat(1.0, 1.4) + cl.time;

		if (i & 1)
		{
			ap->type = pt_blob;
			ap->color = RandomLong(66, 71);
		}
		else
		{
			ap->type = pt_blob2;
			ap->color = RandomLong(150, 155);
		}
#if defined( GLQUAKE )
		ap->packedColor = 0;
#else
		ap->packedColor = hlRGB(host_basepal, ap->color);
#endif

		for (int j = 0; j < 3; j++)
		{
			ap->org[j] = RandomFloat(-16.0, 16.0) + org[j];
			ap->vel[j] = RandomFloat(-256.0, 256.0);
		}
	}
}
...
void R_UserTracerParticle(vec_t* org, vec_t* vel, float life, int colorIndex, float length, byte context, void(*deathfunc)(particle_t*))
{
	if (colorIndex < 0)
	{
...
void R_Blood(vec_t* org, vec_t* dir, int pcolor, int speed)
{
	...
	for (int count = 0, pspeed = speed * 3; count < speed / 2; count++, pspeed -= speed)
	{
...
BEAM* R_BeamEnts(int startEnt, int endEnt, int modelIndex, float life, float width, float amplitude,
...
BEAM* R_BeamEntPoint(int startEnt, vec_t* end, int modelIndex, float life, float width, float amplitude,
	float brightness, float speed, int startFrame, float framerate, float r, float g, float b)
{
	BEAM* p;
...
// SH_CODE: review. two recursive calls, no cycle - DONE
void Noise(float* noise, int divs)
{
	...
	noise[div2] = (noise[0] + noise[divs]) * 0.5 + (float)divs * RandomFloat(-0.125, 0.125);
	if (div2 > 1)
	{
...
void R_DrawSegs(vec_t *source, vec_t *delta, float width, float scale, float freq, float speed, int segments, int flags)
{
	...
	if (flags & FBEAM_SINENOISE)
		...
		noiseidx = int(div * NOISE_DIVS * 0x10000);
	...
	for (seg = 1; seg < segments; seg++)
	{
		...
		if (scale != 0.0f)
		{
			v = gNoise[noiseidx >> 16] * scale;

			if (flags & FBEAM_SINENOISE)
			{
				VectorMA(point, v * sin(fraction * M_PI * length + freq), vup, point);
				// rotate the noise along the perpendicluar axis a bit to keep the bolt from looking diagonal
				VectorMA(point, v * cos(fraction * M_PI * length + freq), vright, point);
			}
			else
			{
				VectorMA(point, v, vup, point);
				VectorMA(point, v * cos(fraction * M_PI * 3.0 + freq), vright, point);
			}
		}
...
void R_DrawDisk(vec_t *source, vec_t *delta, float width, float scale, float freq, float speed, int segments)
{
	...
	vec3_t screenLast, point, screen, tmp, normal;
	int seg;
...
void R_BeamDraw(BEAM* pbeam, float frametime)
{
	...
	if (pbeam->flags & FBEAM_STARTENTITY | FBEAM_ENDENTITY)
	{
		...
		if (pbeam->flags & FBEAM_ENDENTITY)
		{
			...
			// @SH_CODE: should we return if endvisible?
		}
...
particle_t* R_AllocParticle( void( *callback )( particle_t*, float ) )
{
	...
	head = free_particles;
	free_particles = head->next;
	head->next = active_particles;
	active_particles = head;

	head->type = pt_clientcustom;
	head->die = cl.time;
	head->color = 0;
	head->packedColor = 0;
