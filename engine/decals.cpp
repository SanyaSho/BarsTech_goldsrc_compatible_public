...
qboolean Draw_CustomCacheWadInit(int cacheMax, cachewad_t* wad, void* raw, int nFileSize)
{
	...
	// @SH_CODE: lump_p->disksize in build from 08.2019
	if (lump_p->size < 1)
	{
...
void Draw_DecalSetName(int decal, char* name)
{
	...
	Q_strncpy(decal_names[decal], name, sizeof(decal_names[0]) - 1);
...
texture_t* Draw_DecalTexture(int index)
{
	...
	else
	{
		...
		if (pCust == NULL || pCust->bInUse == false || pCust->pInfo == NULL || pCust->pBuffer == NULL)
		{
...
int Draw_DecalIndexFromName(char* name)
{
	...
	{
		if (decal_names[i][0] && !Q_strcmp(tmpName, decal_names[i]))
			return i;
	}
...
{
	return Decal_CountLumps(plist) * sizeof(lumpinfo_t);
}
...
void Decal_MergeInDecals(cachewad_t *pwad, const char *pathID)
{
	...
	if (!decal_wad)
	{
		...
		pwad->basedirs = (char **)Mem_Malloc(sizeof(char*) * 1);
		decal_wad->basedirs[0] = Mem_Strdup(pathID);
	...
	Draw_CacheWadHandler(final, decal_wad->pfnCacheBuild, decal_wad->cacheExtra);
	...
	final->basedirs[0] = Mem_Strdup(decal_wad->basedirs[0]);
	...
	while (plump != NULL)
	{
		...
		final->lumppathindices[lumpcount++] = (plump->breplaced != false);
		Mem_Free(plump);
		plump = pnext;
		lump++;
	}
...
void Decal_Init()
{
	...
	for (i = 0; i < ARRAYSIZE(pszPathID); i++)
	{
		...
		if (i == 0 && hfile == FILESYSTEM_INVALID_HANDLE)
...
qboolean CustomDecal_Validate(void* raw, int nFileSize)
{
	...
	if (fakewad != NULL)
	{
		bret = CustomDecal_Init(fakewad, raw, nFileSize, ~1);
		if (bret)
...
qboolean Draw_ValidateCustomLogo(cachewad_t* wad, unsigned char* data, lumpinfo_t* lump)
{
	...
	if (!tex.width || !tex.height || tex.width > 256 || tex.height > 256)
	{
		Con_Printf(const_cast<char*>(__FUNCTION__ ": Bad cached wad %s\n"), wad->name);
		return FALSE;
	}
...
int Draw_CacheIndex(cachewad_t* wad, char* path)
{
	...
	if (i == wad->cacheCount)
	{
		...
		Q_strncpy(pic->name, path, sizeof(pic->name) - 1);
		pic->name[sizeof(pic->name) - 1] = 0;
	}
	return i;
}
