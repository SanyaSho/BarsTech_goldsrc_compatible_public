...
{
	if( COM_CheckParm( const_cast<char*>("-addons") ) )
		return true;
...
{
	if( COM_CheckParm( const_cast<char*>("-nohdmodels") ) )
		return false;
...
void CheckLiblistForFallbackDir( const char *pGameDir, bool bLanguage, const char *pLanguage, bool bLowViolenceBuild )
{
	...
	if( bFound )
	{
		...
		if( stricmp( pGameDir, szFallback ) )
		{
			...
			{
				FileSystem_AddDirectory( true, "GAME_FALLBACK", "%s/%s_lv", GetBaseDirectory(), szFallback );
			}

			if( BEnableAddonsFolder() )
			{
				FileSystem_AddDirectory( true, "GAME_FALLBACK", "%s/%s_addon", GetBaseDirectory(), szFallback );
			}
			...
			if( bLanguage )
			{
				if( pLanguage )
				{
					FileSystem_AddDirectory( false, "GAME_FALLBACK", "%s/%s_%s", GetBaseDirectory(), szFallback, pLanguage );
					if( !COM_CheckParm( const_cast<char*>("-steam") ) )
					{
						char baseDir[ 4096 ];
						Q_strncpy( baseDir, GetBaseDirectory(), sizeof( baseDir ) );
			...
			if( BEnabledHDAddon() )
			{
				FileSystem_AddDirectory( true, "GAME_FALLBACK", "%s/%s_hd", GetBaseDirectory(), szFallback );
			}

			FileSystem_AddDirectory( false, "GAME_FALLBACK", "%s/%s", GetBaseDirectory(), szFallback );
...
bool FileSystem_AddFallbackGameDir( const char *pGameDir )
{
	...
	{
		char temp[ 4096 ];
		snprintf( temp, sizeof( temp ), "%s/%s_%s", GetBaseDirectory(), pGameDir, language );
...
bool FileSystem_SetGameDirectory( const char *pDefaultDir, const char *pGameDir )
{
	...
	char szBuffer[128];
	int nBufferLen = 127;
	Sys_GetRegKeyValue("Software\\Valve\\Half-Life\\Settings", "User Token 2", szBuffer, nBufferLen, "");

	bLowViolenceBuild = (Q_strlen(szBuffer) > 0) ? true : false;

	if (!bLowViolenceBuild)
	{
		Sys_GetRegKeyValue("Software\\Valve\\Half-Life\\Settings", "User Token 3", szBuffer, nBufferLen, "");
	...
	if( pGameDir || pDefaultDir )
	{
		...
		if( bLowViolenceBuild )
		{
			FileSystem_AddDirectory( true, "GAME", "%s/%s_lv", GetBaseDirectory(), pszModDir);
		}

		if( BEnableAddonsFolder() )
		{
			FileSystem_AddDirectory( true, "GAME", "%s/%s_addon", GetBaseDirectory(), pszModDir );
		}

		if( bLanguage )
		{
			FileSystem_AddDirectory( true, "GAME", "%s/%s_%s", GetBaseDirectory(), pszModDir, language );
		...
		if( BEnabledHDAddon() )
		{
			FileSystem_AddDirectory( true, "GAME", "%s/%s_hd", GetBaseDirectory(), pszModDir );
		}

		FileSystem_AddDirectory( false, "GAME", "%s/%s", GetBaseDirectory(), pszModDir );
		FileSystem_AddDirectory( false, "GAMECONFIG", "%s/%s", GetBaseDirectory(), pszModDir );
		FileSystem_AddDirectory( false, "GAMEDOWNLOAD", "%s/%s_downloads", GetBaseDirectory(), pszModDir );
	...
	if( bLanguage )
	{
		if( bLowViolenceBuild )
		{
			FileSystem_AddDirectory( true, "DEFAULTGAME", "%s/%s_lv", GetBaseDirectory(), pDefaultDir );
		}

		if( COM_CheckParm( const_cast<char*>("-addons") ) || registry->ReadInt( "addons_folder", 0 ) > 0 )
		{
			FileSystem_AddDirectory( true, "DEFAULTGAME", "%s/%s_addon", GetBaseDirectory(), pDefaultDir );
		}

		FileSystem_AddDirectory( true, "DEFAULTGAME", "%s/%s_%s", GetBaseDirectory(), pDefaultDir, language );
	...
	if( BEnabledHDAddon() )
	{
		FileSystem_AddDirectory( true, "DEFAULTGAME", "%s/%s_hd", GetBaseDirectory(), pDefaultDir );
	}

	FileSystem_AddDirectory( false, "BASE", "%s", GetBaseDirectory());
	FileSystem_AddDirectory( true, "DEFAULTGAME", "%s/%s", GetBaseDirectory(), pDefaultDir );
	FileSystem_AddDirectory( false, "PLATFORM", "%s/platform", GetBaseDirectory());
	return true;
}
...
void Host_SetAddonsFolder_f()
{
	if( cls.state != ca_dedicated && Cmd_Argc() == 2 )
	{
		...
		const auto bNewState =
			COM_CheckParm( const_cast<char*>("-addons") ) ||
...
void Host_SetHDModels_f()
{
	if( cls.state != ca_dedicated && Cmd_Argc() == 2 )
	{
		...
		const bool bNewState =
			!COM_CheckParm( const_cast<char*>("-nohdmodels") ) &&
