...
void Host_WriteCustomConfig(void)
{
	...
	if (Cmd_Argc() == 1 || Cmd_Args() == NULL || Cmd_Args()[0] == 0)
	{
		Con_Printf(const_cast<char*>("writecfg: writecfg <filename>"));
		return;
	}
	...
	if (configname[0] == '/'
		...
		|| Q_strstr(configname, "./"))
	{
		Con_Printf(const_cast<char*>("skipping writecfg output, invalid filename %s given (invalid characters in filename)\n"), configname);
	}
	else
	{
		if (host_initialized && cls.state != ca_dedicated)
		{
			...
			{
				COM_DefaultExtension(configname, ".cfg");

				if (Q_stricmp(COM_LastFileExtension(configname), "cfg"))
				{
					Con_Printf(
						const_cast<char*>("skipping writecfg output, invalid filename %s given (wrong file extension, must be .cfg)\n"),
						configname);
					return;
				}

				if ( !Q_stricmp(configname, "config.cfg")
					|| !Q_stricmp(configname, "autoexec.cfg")
					|| !Q_stricmp(configname, "listenserver.cfg")
					|| !Q_stricmp(configname, "server.cfg")
					|| !Q_stricmp(configname, "userconfig.cfg")
					|| !Q_stricmp(configname, "language.cfg")
					|| !Q_stricmp(configname, "violence.cfg")
					|| !Q_stricmp(configname, "banned.cfg")
					|| !Q_stricmp(configname, "listip.cfg")
					|| !Q_stricmp(configname, "skill.cfg")
					|| !Q_stricmp(configname, "game.cfg")
					|| !Q_stricmp(configname, "joystick.cfg")
					|| Q_striprefix(configname, "hw/") == true)
				{
...
void SV_ClientPrintf(const char* fmt, ...)
{
	...
	{
		va_start(va, fmt);
		_vsnprintf(string, ARRAYSIZE(string), fmt, va);
		va_end(va);
...
void SV_BroadcastPrintf(const char* fmt, ...)
{
	...
	va_start(argptr, fmt);
	_vsnprintf(string, ARRAYSIZE(string), fmt, argptr);
	va_end(argptr);
...
qboolean Host_FilterTime(float time)
{
	...
	double flDelta = realtime - oldrealtime;
	...
	if (g_bIsDedicatedServer)
	{
		...
		{
			flTicRate = atof(com_argv[command_line_ticrate + 1]);
		}
...
void Host_Version()
{
	...
	{
		unsigned int iSize = FS_Size(hFile);
		...
		int iProductNameLength = Q_strlen("ProductName=");
		int iPatchVerLength = Q_strlen("PatchVersion=");

		char szSteamVersionId[32];

		//Parse out the version and name.
		for (int i = 0; (pBuffer = COM_Parse(pBuffer)) != NULL && *com_token && i < 2; )
		{
			if (!Q_strnicmp(com_token, "PatchVersion=", iPatchVerLength))
			{
				i++;

				Q_strncpy(gpszVersionString, &com_token[iPatchVerLength], ARRAYSIZE(gpszVersionString) - 1);
				gpszVersionString[ARRAYSIZE(gpszVersionString) - 1] = 0;
			...
			{
				i++;
				Q_strncpy(gpszProductString, &com_token[iProductNameLength], ARRAYSIZE(gpszProductString) - 1);
				gpszProductString[ARRAYSIZE(gpszProductString) - 1] = 0;
			}
		}
...
int Host_Init(quakeparms_t* parms)
{
	...
	if (cls.state != ca_dedicated)
	{
		...
		host_basepal = (unsigned short*)Hunk_AllocName(256 * sizeof(PackedColorVec), const_cast<char*>("palette.lmp"));

		for (int i = 0; i < 256; i++)
		{
			host_basepal[(4 * i) + 0] = pSource[i * 3 + 2];
			host_basepal[(4 * i) + 1] = pSource[i * 3 + 1];
			host_basepal[(4 * i) + 2] = pSource[i * 3 + 0];
