...
bool CheckForCommand()
{
	...
	pszCompleted = Cmd_CompleteCommand(command, !shift_down);

	if (!pszCompleted)
		pszCompleted = Cvar_CompleteVariable(command, !shift_down);

	if (pszCompleted && !Q_strcmp((char*)pszCompleted, command))
		return true;
...
void CompleteCommand()
{
	...
	if( pszCompleted )
	{
		*key_lines[ edit_line + 1 ] = '/';
		...
		key_linepos = Q_strlen( (char*)pszCompleted );
...
void Cbuf_ReplaceSemiColons()
{
	...
	for( char* temp = chat_buffer; *temp; ++temp )
	{
		if( *temp == ';' )
		{
			*temp = ',';
...
SDL_Keycode GetSDLKeycodeFromEngineKey( int iKey )
{
	...
	case K_KP_DEL: code = SDL_SCANCODE_KP_DECIMAL; break;
...
void Key_Message( int key )
{
	int length;
	char* pszEnd;
	void (*pfnAddText)(char*);

	if( key == K_KP_ENTER || key == K_ENTER )
	{
		Cbuf_ReplaceSemiColons();

		if (message_type_is_privileged == false)
			pfnAddText = Cbuf_AddFilteredText;
		else
			pfnAddText = Cbuf_AddText;

		pfnAddText( message_type );
		pfnAddText( const_cast<char*>(" \"") );
		pfnAddText( chat_buffer );
		pfnAddText( const_cast<char*>("\"\n") );
		...
		message_type_is_privileged = true;
		return;
	}
...
//		return;	// non printable
	...
	if( key == K_BACKSPACE )
	{
		if( chat_bufferlen )
		{
			length = Q_UnicodeLength( chat_buffer );
			pszEnd = Q_UnicodeAdvance( chat_buffer, length - 1 );
...
int Key_StringToKeynum(const char* str)
{
	...
	{
		if (!Q_strcasecmp((char*)str, (char*)kn->name))
			return kn->keynum;
	}
	return -1;
}
...
void Key_SetBinding(int keynum, const char* binding)
{
	...
	if (keybindings[keynum])
	{
		...
		if (!Q_strcmp(keybindings[keynum], (char*)binding))
			return;
	...
	Q_strcpy(newBinding, (char*)binding);
	newBinding[l] = '\0';
	keybindings[keynum] = newBinding;

	if (!stricmp(binding, "toggleconsole"))
...
void Key_Unbind_f()
{
	if (Cmd_Argc() != 2)
	{
		Con_Printf(const_cast<char*>("unbind <key> : remove commands from a key\n"));
		return;
	}
	...
	if (b == -1)
	{
		Con_Printf(const_cast<char*>("\"%s\" isn't a valid key\n"), Cmd_Argv(1));
		return;
	}

	if (b == K_ESCAPE)
	{
		Con_Printf(const_cast<char*>("Can't unbind ESCAPE key\n"));
		return;
	}
...
void Key_Escape_f()
{
	if( giSubState & 0x10 )
	{
		Cbuf_AddText( const_cast<char*>("disconnect\n") );
		giActive = DLL_PAUSED;
		giStateInfo = 2;
		giSubState = 1;
		Cbuf_Execute();
...
void Key_Bind_f()
{
	...
	if (c != 2 && c != 3)
	{
		Con_Printf(const_cast<char*>("bind <key> [command] : attach a command to a key\n"));
		return;
	}
	...
	if (c == 2)
	{
		if (keybindings[b])
			...
			Con_Printf(const_cast<char*>("\"%s\" is not bound\n"), Cmd_Argv(1));
		return;
	}
	...
	else
	{
		...
		{
			if (i > 2)
				strncat(cmd, " ", ARRAYSIZE(cmd) - 1 - strlen(cmd));
			strncat(cmd, Cmd_Argv(i), ARRAYSIZE(cmd) - 1 - strlen(cmd));
		}
	}
...
{
	int iLength = Q_strlen( (char*)pBinding );

	RecEngKey_NameForBinding(pBinding);
	...
	for( int i = 0; i < 256; ++i )
	{
		...
		if( ( *keybindings[ i ] == '+' && !strnicmp( keybindings[ i ] + 1, pBinding, iLength ) ) ||
			!strnicmp( keybindings[ i ], pBinding, iLength ) )
		{
...
void Key_Init()
{
	...
	//
	Cmd_AddCommandWithFlags( const_cast<char*>("bind"), Key_Bind_f, CMD_PRIVILEGED );
	Cmd_AddCommandWithFlags( const_cast<char*>("unbind"), Key_Unbind_f, CMD_PRIVILEGED);
	Cmd_AddCommandWithFlags( const_cast<char*>("unbindall"), Key_Unbindall_f, CMD_PRIVILEGED);
	Cmd_AddCommand( const_cast<char*>("escape"), Key_Escape_f );
}
...
void Key_Event( int key, qboolean down )
{
	...
	else
	{
		...
		if (key != K_BACKSPACE && key != K_PAUSE && key != K_PGUP &&
			key != K_KP_PGUP && key != K_PGDN && key != K_KP_PGDN && key_repeats[key] > 1)
			return;	// ignore most autorepeats

		if (key >= 200 && !keybindings[key])
		{
			Con_Printf(const_cast<char*>("%s is unbound.\n"), Key_KeynumToString(key));
		}
	}
		...
		|| ( key_dest == key_game && ( !con_forcedup || !consolekeys[ key ] ) ) )
	{
		...
		if( kb )
		{
			...
			else if( strnicmp( kb, "toggleconsole", 4 ) || !keydown[ K_SHIFT ] || con_shifttoggleconsole.value )
			{
				Cbuf_AddText( kb );
