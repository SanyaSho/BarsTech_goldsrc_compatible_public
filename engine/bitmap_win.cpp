...
void LoadBMP8(FileHandle_t hFile, byte** pPalette, int* nPalette, byte** pImage, int* nWidth, int* nHeight)
{
	...
	*pPalette = nullptr;
	...
	if (FS_Read(&bmfh, sizeof(BITMAPFILEHEADER), 1, hFile) == sizeof(BITMAPFILEHEADER) && !bmfh.bfReserved1 && !bmfh.bfReserved2)
	{
		if (bmfh.bfSize <= FS_Size(hFile) && FS_Read(&bmih, sizeof(bmih), 1, hFile) == sizeof(bmih)
			...
			&& bmih.biHeight > 0)
		{
			...
			if (FS_Read(rgrgbPalette, sizeof(DWORD) * bmih.biClrUsed, 1, hFile) != sizeof(DWORD) * bmih.biClrUsed)
				goto GetOut;

			ULONG cbBmpBitsa = bmfh.bfSize - FS_Tell(hFile);

			DWORD biTrueWidth = ((bmih.biWidth + sizeof(RGBTRIPLE) & ~3));
			...
			if (biPixelCount >= (DWORD)bmih.biHeight && biPixelCount <= cbBmpBitsa)
			{
				...
				*pPalette = pTempPal;
				...
				if (FS_Read(pOutput, cbBmpBitsa, 1, hFile) != cbBmpBitsa)
				{
					Mem_Free(pTempPal);
					pTempPal = nullptr;
...
byte* LoadBMP16(FileHandle_t fin, qboolean is15bit)
{
	return NULL;
}
