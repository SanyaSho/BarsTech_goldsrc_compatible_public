...
ENTITYINIT GetEntityInit( const char* pClassName )
{
	...
	return result;*/
	return reinterpret_cast<ENTITYINIT>(GetIOFunction(pClassName));
}
...
void LoadEntityDLLs( const char* szBaseDir )
{
	...
	if( hLibListFile )
	{
		...
		while( 1 )
		{
			...
			if( stricmp( szKey, GAMEDLL_KEY ) )
			{
				DLL_SetModKey( &gmodinfo, szKey, szValue );
			}
			else
			{
				const int iDllOverride = COM_CheckParm( const_cast<char*>("-dll") );

				if( iDllOverride && iDllOverride < com_argc - 1 )
				{
					Q_strncpy( szValue, (char*)com_argv[ iDllOverride + 1 ], ARRAYSIZE( szValue ) );
				}
				...
				if( strstr( szValue, DEFAULT_SO_EXT ) )
				{
...
//
//extern "C" void Sys_PushFPCW_SetHigh();
//
//extern "C" void Sys_PopFPCW();
//
//extern "C" void MaskExceptions();

// extern "C" long pushed_cw, full_cw;
...
//	_asm fnstcw ds:word ptr[pushed_cw]
//	_asm fldcw ds:word ptr[full_cw]
...
#ifdef WIN32
HMODULE Sys_LoadWindowsDLL(LPCSTR pszPath)
{
	if (pszPath == NULL)
		return NULL;

	FS_GetLocalCopy(pszPath);
