...
{
	char** check = &sv.model_precache[0];
	int i = 0;
...
int PF_CreateInstancedBaseline(int classname, entity_state_t* baseline)
{
	...
	Q_memcpy(&bls->baseline[bls->number], baseline, sizeof(entity_state_t));
	bls->number += 1;
	return bls->number;
}
...
void PF_ForceUnmodified(FORCE_TYPE type, float* mins, float* maxs, const char* filename)
{
	...
	if (sv.state == ss_loading)
	{
		...
		{
			if (!Q_strcasecmp(cnode->filename, (char*)filename))
				return;

			++cnode;
			++i;

			if (i >= MAX_CONSISTENCY_LIST)
		...
		if (mins)
		{
			VectorCopy(mins, cnode->mins);
		}
		if (maxs)
		{
			VectorCopy(maxs, cnode->maxs);
		}
	}
	else
	{
		i = 0;
		consistency_t* cnode = sv.consistency_list;
		while (!cnode->filename || Q_strcasecmp(cnode->filename, filename))
		{
			++cnode;
			++i;
			if (i >= MAX_CONSISTENCY_LIST)
