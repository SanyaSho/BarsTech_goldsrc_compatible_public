...
void Voice_EndChannel(int iChannel, bool bTellVoiceSE);
...
cvar_t voice_dsound = { const_cast<char*>("voice_dsound"), const_cast<char*>("0") };
cvar_t voice_avggain = { const_cast<char*>("voice_avggain"), const_cast<char*>("0.5") };
cvar_t voice_maxgain = { const_cast<char*>("voice_maxgain"), const_cast<char*>("5") };
cvar_t voice_scale = { const_cast<char*>("voice_scale"), const_cast<char*>("1"), FCVAR_ARCHIVE };

cvar_t voice_loopback = { const_cast<char*>("voice_loopback"), const_cast<char*>("0") };
cvar_t voice_fadeouttime = { const_cast<char*>("voice_fadeouttime"), const_cast<char*>("0.1") };	// It fades to no sound at the tail end of your voice data when you release the key.

// Debugging cvars.
cvar_t voice_profile = { const_cast<char*>("voice_profile"), const_cast<char*>("0") };
cvar_t voice_showchannels = { const_cast<char*>("voice_showchannels"), const_cast<char*>("0") };	// 1 = list channels
// 2 = show timing info, etc
cvar_t voice_showincoming = { const_cast<char*>("voice_showincoming"), const_cast<char*>("0") };	// show incoming voice data

cvar_t voice_enable = { const_cast<char*>("voice_enable"), const_cast<char*>("1"), FCVAR_ARCHIVE };
...
cvar_t voice_forcemicrecord = { const_cast<char*>("voice_forcemicrecord"), const_cast<char*>("1"), FCVAR_ARCHIVE };
...
void VoiceTweak_SetControlFloat(VoiceTweakControl iControl, float value)
{
	...
	{
		g_pMixerControls->SetValue_Float(IMixerControls::Control::MicBoost, value);
	}
	else if (iControl == OtherSpeakerScale)
	{
		voice_scale.value = value;
	}
}
...
float VoiceTweak_GetControlFloat(VoiceTweakControl iControl)
{
	...
	{
		return voice_scale.value;
	}
	else if (iControl == MicBoost)
	{
		float value = 1;
		g_pMixerControls->GetValue_Float(IMixerControls::Control::MicBoost, value);
		return value;
	}
	else
	{
...
void CVoiceChannel::Init(int nEntity)
{
	...
	m_AutoGain.Reset(128, voice_maxgain.value, voice_avggain.value, voice_scale.value);
}
	...
	)
{
	...
	if (nSamplesGotten < sampleCount)
	{
		...
		Q_memset(&pCopyBuf[nSamplesGotten], 0, (wantedSampleCount - nSamplesGotten) * BYTES_PER_SAMPLE);
	...
	if (voice_showchannels.value >= 2)
	{
		Con_Printf(const_cast<char*>("Voice - mixed %d samples from channel %d\n"), nSamplesGotten, iChannel);
	}
...
extern "C" void Voice_RegisterCvars()
{
...
public:
	inline void		Start()
	{
		if (voice_profile.value)
			m_StartTime = Sys_FloatTime();
	}

	inline void		End(double *out)
	{
		if (voice_profile.value)
			*out += Sys_FloatTime() - m_StartTime;
	}
...
static qboolean	g_bLocalPlayerTalkingAck = false;
...
static qboolean		g_bVoiceRecording = false;	// Are we recording at the moment?
static bool			g_bUsingSteamVoice = true;
...
qboolean Voice_Init( const char* pCodecName, int quality )
{
	...
	g_bUsingSteamVoice = SteamUser() != NULL;

	if (!g_bUsingSteamVoice && pCodecName[0])
	{
		// Get the voice input device.
		int rate = g_VoiceSampleFormat.nSamplesPerSec;

		if (!voice_dsound.value)
		{
			g_pVoiceRecord = CreateVoiceRecord_WaveIn(rate);

			if (!g_pVoiceRecord)
			{
				Con_Printf(const_cast<char*>("Unable to initialize waveIn. Trying DirectSound."));
		...
		else
		{
			...
			if (!g_pVoiceRecord)
			{
				Con_Printf(const_cast<char*>("Unable to initialize DirectSoundCapture. Trying waveIn."));
		...
		if (!g_pVoiceRecord)
			Con_Printf(const_cast<char*>("Unable to initialize wave input. You won't be able to speak"));

		// Get the codec.
		char dllName[256];
		_snprintf(dllName, sizeof(dllName), "%s.dll", pCodecName);
		CreateInterfaceFn createCodecFn;
		g_hVoiceCodecDLL = FS_LoadLibrary(dllName);

		if (!g_hVoiceCodecDLL)
		{
			Con_Printf(const_cast<char*>("Unable to load voice codec DLL %s.  Voice disabled.\n"), pCodecName);
			Voice_Deinit();
			return false;
		}

		if (!(createCodecFn = Sys_GetFactory(g_hVoiceCodecDLL)))
		{
			Con_Printf(const_cast<char*>("Unable to get a factory for voice codec %s.  Voice disabled.\n"), pCodecName);
			Voice_Deinit();
			return false;
		}

		if (!(g_pEncodeCodec = (IVoiceCodec*)createCodecFn(pCodecName, NULL)))
		{
			Con_Printf(const_cast<char*>("Unable to create interface for voice codec %s.  Voice disabled.\n"), pCodecName);
			Voice_Deinit();
			return false;
		}
		...
		{
			Con_Printf(const_cast<char*>("Unable to initialize voice codec %s.  Voice disabled.\n"), pCodecName);
			Voice_Deinit();
			return false;
		}
		...
		for (int i = 0; i < VOICE_NUM_CHANNELS; i++)
		{
			...
			if (!(pChannel->m_pVoiceCodec = (IVoiceCodec*)createCodecFn(pCodecName, NULL)) || !pChannel->m_pVoiceCodec->Init(quality))
			{
				Con_Printf(const_cast<char*>("Unable to get voice codec (%s) interface for channel %d.  Voice disabled.\n"), pCodecName, i);
				Voice_Deinit();
				return false;
			}
		}
...
void Voice_EndChannel(int iChannel, bool bTellVoiceSE)
{
	...
	if (g_VoiceChannels[iChannel].m_iEntity >= 0)
	{
		ClientDLL_VoiceStatus(iChannel, bTellVoiceSE);
		VoiceSE_CloseMouth(g_VoiceChannels[iChannel].m_iEntity);
	}

	g_VoiceChannels[iChannel].m_iEntity = -1;
}

void Voice_NotifyFreeChannel(int entchan)
{
	Voice_EndChannel(entchan, false);
}
...
void Voice_Deinit()
{
	...
	if (g_hVoiceCodecDLL)
	{
		Sys_UnloadModule(g_hVoiceCodecDLL);
...
{
	return voice_loopback.value != 0.0f;
}

void Voice_LocalPlayerTalkingAck()
{
	if (!g_bLocalPlayerTalkingAck)
		ClientDLL_VoiceStatus(VOICE_CHANNEL_IN_TWEAK_MODE, true);

	g_bLocalPlayerTalkingAck = true;
	g_LocalPlayerTalkingTimeout = 0.0;
}
...
void Voice_Idle(float frametime)
{
	...
	if (g_bLocalPlayerTalkingAck)
	{
		...
		if (g_LocalPlayerTalkingTimeout > LOCALPLAYERTALKING_TIMEOUT)
		{
			...
			// Tell the client DLL.
			ClientDLL_VoiceStatus(VOICE_CHANNEL_IN_TWEAK_MODE, FALSE);
		}
	}

	// Precalculate these to speedup the voice fadeout.
	g_nVoiceFadeSamples = max((int)(voice_fadeouttime.value * VOICE_OUTPUT_SAMPLE_RATE), 2);
	...
	for (int i = 0; i < VOICE_NUM_CHANNELS; i++)
	{
		...
		{
			if (pChannel->m_bStarved && !g_bInTweakMode)
			{
	...
	if (nActive == 0 && !g_bInTweakMode)
		VoiceSE_EndOverdrive();

	VoiceSE_Idle(frametime);

	// voice_showchannels.
	if (voice_showchannels.value >= 1)
	{
		for (int i = 0; i < VOICE_NUM_CHANNELS; i++)
		{
			CVoiceChannel *pChannel = &g_VoiceChannels[i];
...
{
	return g_bVoiceRecording == true && !g_bInTweakMode;
}
...
static bool VoiceRecord_Start()
{
	if (g_bUsingSteamVoice && SteamUser())
	{
...
int Voice_GetCompressedData(char *pchDest, int nCount, qboolean bFinal)
{
	...
	if (g_bUsingSteamVoice)
	{
		...
		if (SteamUser()->GetAvailableVoice(&cbCompressed, &cbUncompressed, g_pUncompressedFileData != NULL ? VOICE_OUTPUT_SAMPLE_RATE : 0) == k_EVoiceResultOK)
		{
			SteamUser()->GetVoice(true, pchDest, nCount, &cbCompressedWritten, g_pUncompressedFileData != NULL, g_pUncompressedFileData, MAX_WAVEFILEDATA_LEN - g_nUncompressedDataBytes, &cbUncompressedWritten, VOICE_OUTPUT_SAMPLE_RATE);
	...
	else if (g_pVoiceRecord && pCodec)
	{
		short tempData[4096 * BYTES_PER_SAMPLE];
		...
		if (g_pMicInputFileData)
		{
			...
			gotten = min((int)sizeof(tempData) / BYTES_PER_SAMPLE, min(nShouldGet, (g_nMicInputFileBytes - g_CurMicInputFileByte) / BYTES_PER_SAMPLE));
...
int Voice_AddIncomingData(int nChannel, const char *pchData, int nCount, int iSequenceNumber)
{
	...
	if (!(pChannel = GetVoiceChannel(nChannel)))
	{
		return(0);
	}
	...
	// Decompress.
	char decompressed[4096 * BYTES_PER_SAMPLE];
	unsigned int cb = 0;
	int nDecompressed;

	if (g_bUsingSteamVoice)
	{
		EVoiceResult result = SteamUser()->DecompressVoice(pchData, nCount, decompressed, sizeof(decompressed), &cb, VOICE_OUTPUT_SAMPLE_RATE);

		if (result != k_EVoiceResultOK || !cb)
			return 0;

		nDecompressed = cb / BYTES_PER_SAMPLE;
	}
	else
	{
	...
	if (g_bInTweakMode)
	{
		...
		{
			int high = abs(((short*)decompressed)[i]);
			if (high > g_VoiceVolume)
				g_VoiceVolume = high;
		}
		g_VoiceVolume &= (word)~511;
	}
	...
	if (voice_showincoming.value != 0)
	{
		Con_Printf(const_cast<char*>("Voice - %d incoming samples added to channel %d\n"), nDecompressed, nChannel);
	}
