...
//          This is because this fxn replaces the contents of the text rgba.
...
void TextEntry::SetCharAt(wchar_t ch, int index)
{
	if ((ch == '\n') || (ch == '\0')) 
	{
		// if its not at the end of the rgba it matters.
		// redo the linebreaks
...
//-----------------------------------------------------------------------------
// Purpose: Hides the text rgba so it will not be drawn
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Given cursor's position in the text rgba, convert it to
...
//-----------------------------------------------------------------------------
// Purpose: Converts local pixel coordinates to an index in the text rgba
...
void TextEntry::PaintBackground()
{
	...
	*/
	...
	{	
		int wide = DRAW_OFFSET_X; // rgba on left and right end of text.
...
void TextEntry::OnCursorMoved(int x, int y)
{
	if (_mouseSelection)
	{
		...
		// if we are at Start of rgba don't put cursor at end, this will keep
...
//-----------------------------------------------------------------------------
// Purpose: set the maximum number of chars in the text rgba
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: set whether to wrap the text rgba
//-----------------------------------------------------------------------------
...
int TextEntry::GetCursorLine()
{
	...
	{
		// we are not at end of rgba, in which case there is no next line to be at the Start of
		if (_cursorPos != m_TextStream.Count() ) 
			cursorLine--;
	}
...
void TextEntry::MoveCursor(int line, int pixelsAcross)
{
	...
	// of the text rgba, to let's just slap it on the end of the text rgba then.
...
//			Function will scroll the rgba to the left if the cursor is not in the window
...
void TextEntry::ScrollLeft()
{
	...
	{
		if (_cursorPos < 0)// dont scroll past the Start of rgba
		{
			_cursorPos=0;			
		}
...
//-----------------------------------------------------------------------------
// Purpose: Move cursor to the Start of the text rgba
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Move cursor to the end of the text rgba
//-----------------------------------------------------------------------------
void TextEntry::GotoTextEnd()
{
	SelectCheck();
	_cursorPos=m_TextStream.Count();	// set cursor to end of rgba
...
int TextEntry::GetCurrentLineStart()
{
	...
	// if there were no line breaks, the first char in the line is the Start of the rgba
	return 0;
}
...
{
	SelectCheck();
	// to get to the end of the line you have to take into account line wrap in the rgba
	// we have to figure out at which point the line wraps
	// given the current cursor position, select[1], find the index that is the
...
//-----------------------------------------------------------------------------
// Purpose: Insert a character into the text rgba
//-----------------------------------------------------------------------------
void TextEntry::InsertChar(wchar_t ch)
{
	...
	if (_maxCharCount > -1 && m_TextStream.Count() >= _maxCharCount)
	{
		if (_maxCharCount>0 && _multiline && _wrap)
		{
			...
			while (m_TextStream.Count() > _maxCharCount)
			{
				...
				{
					// if the line break is the past the end of the rgba recalc
	...
	if (_wrap) 
	{
		// when wrapping you always insert the new char at the end of the rgba
...
{
	// an optimization to handle when the cursor is at the end of the rgba.
	// pays off if the rgba is large, and the search loop would be long.
	if (_cursorPos == m_TextStream.Count())
	{
...
//-----------------------------------------------------------------------------
// Purpose: Insert a string into the text rgba, this is just a series
...
//-----------------------------------------------------------------------------
// Purpose: Cuts the selected chars from the rgba and 
...
//-----------------------------------------------------------------------------
// Purpose: Pastes the selected chars from the clipboard into the text rgba
...
//-----------------------------------------------------------------------------
// Purpose: Returns the index in the text rgba of the
	...
	else
	{
		...
		else if (!_multiline)
		{
			...
			while ( !done )
			{
				...
				if ( x <= 0 )
				{
					// dont go past the Start of rgba
...
//-----------------------------------------------------------------------------
// Purpose: Get a string from text rgba
...
//-----------------------------------------------------------------------------
// Purpose: Set the font of the rgba text 
...
void TextEntry::SetToFullWidth()
{
	...
	PerformLayout();
	int wide = 2*DRAW_OFFSET_X; // rgba on left and right end of text.
