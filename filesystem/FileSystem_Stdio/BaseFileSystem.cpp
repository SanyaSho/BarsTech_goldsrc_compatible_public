...
FileHandle_t CBaseFileSystem::FindFile(CSearchPath *path, const char *pFileName, const char *pOptions, bool bFromCache)
{
	...
	if (path->IsPackFile())
	{
		char *temp = (char *)alloca((strlen(pFileName) + 1) * sizeof(char));
	...
	else if ((!strchr(pOptions, 'w') && !strchr(pOptions, '+')) || path->IsAllowWrite())
	{
		int len = strlen(path->GetPath().String()) + strlen(pFileName);
		char *pTmpFileName = (char *)alloca((len + 1) * sizeof(char));
		strcpy(pTmpFileName, path->GetPath().String());
		strcat(pTmpFileName, pFileName);
		...
		fh->m_nFileTime     = max(buf.st_ctime, buf.st_mtime);
		fh->m_bPack         = false;
		fh->m_bErrorFlagged = false;
	}
...
FileHandle_t CBaseFileSystem::Open(const char *pFileName, const char *pOptions, const char *pathID)
{
	...
	CSearchPath *searchPath = GetWritePath(pathID);
	int len = strlen(searchPath->GetPath().String()) + strlen(pFileName);
	char *pTmpFileName = (char *)alloca((len + 1) * sizeof(char));

	strcpy(pTmpFileName, searchPath->GetPath().String());
	strcat(pTmpFileName, pFileName);
...
int CBaseFileSystem::FPrintf(FileHandle_t file, char *fmt, ...)
{
	CFileHandle *fh = reinterpret_cast<CFileHandle *>(file);
	if (!fh) {
		Warning((FileWarningLevel_t)-1, "FS:  Tried to FS: fprintf NULL file handle!\n");
		return 0;
	}

	if (!fh->m_pFile) {
		Warning((FileWarningLevel_t)-1, "FS:  Tried to FS: fprintf NULL file pointer inside valid file handle!\n");
		return 0;
	}
...
{
	CSearchPath *searchPath = GetWritePath(pathID);
	int len = strlen(searchPath->GetPath().String()) + strlen(pRelativePath);
	char *pTmpFileName = (char *)alloca((len + 1) * sizeof(char));

	strcpy(pTmpFileName, searchPath->GetPath().String());
	strcat(pTmpFileName, pRelativePath);
	...
	for (int i = 0; i < m_SearchPaths.Count(); i++)
	{
		...
		if (pSearchPath->IsPackFile())
		{
			...
			if (searchresult != pSearchPath->m_PackFiles.InvalidIndex())
			{
				...
				snprintf(pLocalPath, localPathBufferSize - 1, "%s%s", pSearchPath->GetPath().String(), pTmpFileName);
				pLocalPath[localPathBufferSize - 1] = '\0';
				FixSlashes(pLocalPath);
				return pLocalPath;
			}
		...
		{
			int len = strlen(pSearchPath->GetPath().String()) + strlen(pFileName);
			char *pTmpFileName = (char *)alloca((len + 1) * sizeof(char));
			strcpy(pTmpFileName, pSearchPath->GetPath().String());
			strcat(pTmpFileName, pFileName);
			FixSlashes(pTmpFileName);
...
bool CBaseFileSystem::AddPackFileFromPath(const char *pPath, const char *pakfile, bool bCheckForAppendedPack, const char *pathID)
{
	...
	snprintf(fullpath, sizeof(fullpath), "%s%s", pPath, pakfile);
	FixSlashes(fullpath);
	...
	if (bCheckForAppendedPack)
	{
		...
		if (strcmp(appended.id, "PACKAPPE") != 0)
		{
...
bool CBaseFileSystem::PreparePackFile(CSearchPath &packfile, int64_t offsetofpackinmetafile)
{
	...
	if (numpackfiles > 0)
	{
		...
		if (!newfiles) {
			Warning((FileWarningLevel_t)-1, "%s out of memory allocating directory for %i files\n", packfile.GetPath().String(), numpackfiles);
			return false;
		}
...
void CBaseFileSystem::AddPackFiles(const char *pPath)
{
	...
	for (pakcount = 0; ; pakcount++)
	{
		...
		sprintf(pakfile, "pak%i.pak", pakcount);
...
bool IsWildCardMatch(const char *wildcardString, const char *stringToCheck)
{
	...
	if (!strcmp(stringToCheck, ".") || !strcmp(stringToCheck, ".."))
	{
		// ignore containing the levels directory
		return false;
	}

	if (!strcmp(wildcardString, "*.*") || !strcmp(wildcardString, "*"))
	{
		// matches everything
		return true;
	}
...
const char *CBaseFileSystem::FindFirst(const char *pWildCard, FileFindHandle_t *pHandle, const char *pathID)
{
	...
	strcpy(pFindData->m_WildCardString.Base(), pWildCard);
...
const char *CBaseFileSystem::FindFirstHelper(const char *pWildCard, FileFindHandle_t *pHandle, int searchPath, FindData_t *pFindData)
{
	...
	{
		int len = strlen(pSearchPath->GetPath().String()) + pFindData->m_WildCardString.Size();
		char *pTmpFileName = (char *)alloca((len + 1) * sizeof(char));
		strcpy(pTmpFileName, pSearchPath->GetPath().String());
		strcat(pTmpFileName, pFindData->m_WildCardString.Base());
		FixSlashes(pTmpFileName);
...
bool CBaseFileSystem::FileInSearchPaths(const char *pSearchWildcard, const char *pFileName, int minSearchPathID, int maxSearchPathID)
{
	...
	strncpy(pFileNameWithPath, pSearchWildcard, pathStrLen);
	...
	for (int i = minSearchPathID; i <= maxSearchPathID; i++)
	{
		...
		if (!pSearchPath->IsPackFile())
		{
			int len = strlen(pFileNameWithPath) + strlen(pSearchPath->GetPath().String());
			char *fullFilePath = (char *)alloca((len + 1) * sizeof(char));
			strcpy(fullFilePath, pSearchPath->GetPath().String());
...
bool CBaseFileSystem::FindNextFileHelper(FindData_t *pFindData)
{
	if (m_SearchPaths[pFindData->m_CurrentSearchPathID].IsPackFile())
	{
		...
		if (file)
		{
			char *pFileNameNoPath = const_cast<char *>(strrchr(file, CORRECT_PATH_SEPARATOR));
			if (pFileNameNoPath)
			{
	...
	for(; pFindData->m_CurrentSearchPathID < c; pFindData->m_CurrentSearchPathID++)
	{
		...
		if (pSearchPath->IsPackFile())
		{
			...
			if (file)
			{
				Q_strlcpy(pFindData->m_FindData.cFileName, strrchr(file, CORRECT_PATH_SEPARATOR) + 1);
				FixSlashes(pFindData->m_FindData.cFileName);
				return true;
			}
		}
...
void CBaseFileSystem::Warning(FileWarningLevel_t level, const char *fmt, ...)
{
	...
	va_start(argptr, fmt);
	vsnprintf(warningtext, sizeof(warningtext), fmt, argptr);
	va_end(argptr);
...
bool CBaseFileSystem::FullPathToRelativePath(const char *pFullpath, char *pRelative)
{
	...
	for (int i = 0; i < m_SearchPaths.Count() && !success; i++)
	{
		...
		char *searchbase = new char [strlen(pSearchPath->GetPath().String()) + 1];
		strcpy(searchbase, pSearchPath->GetPath().String());
		...
		int baselen = strlen(searchbase);
		if (!strnicmp(searchbase, inpath, baselen))
		{
			success = true;
			strcpy(pRelative, &inpath[baselen]);
		}
