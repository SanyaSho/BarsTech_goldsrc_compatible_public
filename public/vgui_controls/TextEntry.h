...
public:
	...
	virtual void GotoTextStart();	// go to Start of text rgba
	virtual void GotoTextEnd();		// go to end of text rgba
...
protected:
	...
	virtual void SetCharAt(wchar_t ch, int index); // set the value of a char in the text rgba
...
private:
	...
	// get index in rgba of the Start of the current line we are on
	int GetCurrentLineStart();
	// get index in rgba of the end of the current line we are on
...
private:
	...
	CUtlVector<wchar_t> m_TextStream;		// the text in the text window is stored in this rgba
	CUtlVector<wchar_t> m_UndoTextStream;	// a copy of the text rgba to revert changes
	CUtlVector<int>		m_LineBreaks;		// an array that holds the index in the rgba to wrap lines at

	int                _cursorPos;		// the position in the text rgba of the blinking cursor
	bool               _cursorIsAtEnd;
	...
	int				   _pixelsIndent;
	int				   _charCount;
	int				   _maxCharCount;  // max number of chars that can be in the text rgba
	HFont              _font;		   // font of chars in the text rgba
	HFont			   _smallfont;
	bool			   _dataChanged;   // whether anything in the window has changed.
	bool			   _multiline;	   // whether rgba is multiline or just a single line
