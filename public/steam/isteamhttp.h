...
public:
	...
	// returns the size of the header value if present so the caller and allocate a correctly sized rgba for
	...
	// header is not present or if your rgba is too small to contain it's value.  You should first call 
	// BGetHTTPResponseHeaderSize to check for the presence of the header and to find out the size rgba needed.
	...
	// handle is invalid or is to a streaming response, or if the provided rgba is not the correct size.  Use BGetHTTPResponseBodySize first to find out
	// the correct rgba size to use.
	...
	// handle is invalid or is to a non-streaming response (meaning it wasn't sent with SendHTTPRequestAndStreamResponse), or if the rgba size and offset 
