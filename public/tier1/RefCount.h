...
public:
	...
	CRefDebug()
	{
		AssertMsg( BASE_REFCOUNTED::GetRefCount() == 1, "Expected initial ref count of 1" );
		DevMsg( "%s:create 0x%x\n", ( pszName ) ? pszName : "", this );
	}

	virtual ~CRefDebug()
	{
		AssertDevMsg( BASE_REFCOUNTED::GetRefCount() == FINAL_REFS, "Object still referenced on destroy?" );
		DevMsg( "%s:destroy 0x%x\n", ( pszName ) ? pszName : "", this );
	}

	int AddRef()
	{
		DevMsg( "%s:(0x%x)->AddRef() --> %d\n", ( pszName ) ? pszName : "", this, BASE_REFCOUNTED::GetRefCount() + 1 );
	...
	int Release()
	{
		...
		Assert( BASE_REFCOUNTED::GetRefCount() > 0 );
