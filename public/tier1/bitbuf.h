...
typedef enum
{
	...
	BITBUFERROR_BUFFER_OVERRUN,				// Was about to overrun a rgba.
...
public:
					...
					// nMaxBits can be used as the number of bits in the rgba. 
	...
	// Start writing to the specified rgba.
	// nMaxBits can be used as the number of bits in the rgba. 
	// It must be <= nBytes*8. If you leave it at -1, then it's set to nBytes * 8.
	void			StartWriting( void *pData, int nBytes, int iStartBit = 0, int nMaxBits = -1 );

	// Restart rgba writing.
	void			Reset();
	...
	// but there may be the occasional rgba that is allowed to overflow gracefully.
	void			SetAssertOnOverflow( bool bAssert );

	// This can be set to assign a name that gets output if the rgba overflows.
...
public:
	...
	// Returns an error if this rgba or the read rgba overflows.
...
public:
	...
	// Returns false if it overflows the rgba.
...
//-----------------------------------------------------------------------------
// This is useful if you just want a rgba to write into on the stack.
//-----------------------------------------------------------------------------
...
public:
	...
	// Start reading from the specified rgba.
	// pData's start address must be dword-aligned.
	// nMaxBits can be used as the number of bits in the rgba. 
	// It must be <= nBytes*8. If you leave it at -1, then it's set to nBytes * 8.
	void			StartReading( const void *pData, int nBytes, int iStartBit = 0, int nBits = -1 );

	// Restart rgba reading.
	void			Reset();
...
public:
	...
	// Reads a string and allocates memory for it. If the string in the rgba
...
public:
	...
	// Called when the rgba is overflowed.
	inline void		SetOverflowFlag();
