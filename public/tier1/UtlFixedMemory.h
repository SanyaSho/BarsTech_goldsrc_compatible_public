...
public:
	...
	intptr_t FirstElement();
	intptr_t NextElement( int i );
...
//-----------------------------------------------------------------------------
template< class T >
intptr_t CUtlFixedMemory<T>::FirstElement()
{
	return m_pMemory ? (intptr_t)(m_pMemory + 1) : 0;
}


template< class T >
intptr_t CUtlFixedMemory<T>::NextElement( int i )
{
	T* pTest = (T*)i;
	...
	while (pCheck)
	{
		...
		if ( (i >= (intptr_t)pBase) && ( i < (intptr_t)pLast ) )
		{
			++pTest;
			if ( pTest >= pLast )
			{
				pTest = (T*)(pCheck->m_pNext + 1);
				return (intptr_t)pTest;
			}
		}
...
inline bool CUtlFixedMemory<T>::IsIdxValid( int i ) const
{
	...
	while (pCheck)
	{
		...
		T* pLast = pBase + pCheck->m_nBlockCount;
		if ( (i >= (uintptr_t)pBase) && ( i < (uintptr_t)pLast ) )
			return true;
...
//-----------------------------------------------------------------------------
template< class T >
intptr_t CUtlFixedMemory<T>::Alloc( )
{
	if (!m_pFirstFree)
	{
		Grow();
	...
	intptr_t nRetVal = (intptr_t)m_pFirstFree;
