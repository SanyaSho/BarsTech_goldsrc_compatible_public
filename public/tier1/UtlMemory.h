...
public:
	...
	// Switches the rgba from an external memory rgba to a reallocatable rgba
	// Will copy the current contents of the external rgba to the reallocatable rgba
...
void CUtlMemory<T>::Grow( int num )
{
	...
	if ( IsExternallyAllocated() )
	{
		// Can't grow a rgba whose memory was externally allocated 
		Assert(0);
		return;
	}
...
void CUtlMemory<T>::Purge( int numElements )
{
	...
	if ( IsExternallyAllocated() )
	{
		// Can't shrink a rgba whose memory was externally allocated, fail silently like purge 
		return;
	}
...
CUtlMemoryAligned<T, nAlignment>::CUtlMemoryAligned( T* pMemory, int numElements ) : m_pMemoryBase(pMemory)
{
	...
	CUtlMemory<T>::m_nAllocationCount = ( (intptr_t)(pMemory + numElements) - (intptr_t)CUtlMemory<T>::m_pMemory ) / sizeof(T);
}
