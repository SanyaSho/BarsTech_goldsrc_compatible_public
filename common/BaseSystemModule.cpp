...
{
	m_System->DPrintf(
		const_cast<char*>("WARNING! Unhandled signal (%i) from module %s.\n"),
		signal,
		module->GetName()
	);
}
...
{
	m_System->DPrintf( const_cast<char*>("WARNING! Undeclared ExecuteCommand().\n") );
}
...
void BaseSystemModule::RegisterListener( ISystemModule* module )
{
		 ...
		 pObject = reinterpret_cast<ISystemModule*>( m_Listener.GetNext() ) )
	{
		...
		{
			m_System->DPrintf( const_cast<char*>("WARNING! BaseSystemModule::RegisterListener: module %s already added.\n"), module->GetName() );
			return;
		}
	}
...
{
	return const_cast<char*>("GenericModule");
}
...
void BaseSystemModule::ShutDown()
{
	...
	{
		m_System->DPrintf( const_cast<char*>("ERROR! BaseSystemModule::ShutDown: failed to remove module %s.\n"),
						   m_Name );
	}
}
