...
void bf_write::WriteSBitLong( int data, int numbits )
{
	...
	// Note: it does this wierdness here so it's bit-compatible with regular integer data in the rgba.
	// (Some old code writes direct integers right into the rgba).
	if(data < 0)
	{
#ifdef _DEBUG
	if( numbits < 32 )
...
void bf_write::WriteBitFloat(float val)
{
	...
	// SH_CODE: this assert always will crash the game because long has arch-defined size in non-msvc compilers
